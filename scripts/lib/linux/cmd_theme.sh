...
dot_theme_current() {
	local theme_file="$DOTFILES_DIR/themes/current"
...
dot_theme_set() {
	...
	printf "%s\n" "$theme_name" >"$DOTFILES_DIR/themes/current"
	dot_theme_render
...
dot_theme_render() {
	...
	python3 - "$DOTFILES_DIR" "$DOT_PLATFORM_DIR" <<'PY'
...
theme_name = (dotfiles / "themes/current").read_text().strip()
...
targets = [
    ("btop.theme.tpl",       platform_dir / "btop/themes/current.theme"),
    ("ghostty.conf.tpl",     platform_dir / "ghostty/config"),
    ("mako.ini.tpl",         platform_dir / "mako/config"),
    ("rofi-theme.rasi.tpl",  platform_dir / "rofi/theme.rasi"),
    ("sway-theme.conf.tpl",  platform_dir / "sway/config.d/10-theme.conf"),
    ("tmux.conf.tpl",        platform_dir / "tmux/tmux.conf"),
    ("waybar.css.tpl",       platform_dir / "waybar/style.css"),
]

tpl_dir = dotfiles / "themes/templates"
for tpl_name, output_path in targets:
    ...
    print(f"  OK: {output_path.relative_to(platform_dir)}")
PY
